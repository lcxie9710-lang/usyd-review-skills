...
4. **学校政策优先**:如果某门课的老师明确关闭了转录或下载,工具会如实报告无内容——请尊重该设置,不要尝试绕过。
...
```bash
git clone <your-github>/usyd-review-skills
# 两个 skill 各自独立,改哪个就进哪个目录
```

- 提交前检查:不要在 skill 里留下**真实课程内容、token、个人身份信息**
- `sydney-canvas` 依赖仅 `requests`(已最小化,HTML 解析用标准库)
